...
// @title MuSES - Museum Management API
// @version 1.0
// @description Simple REST API in order to properly manage a museum in the system.
...
func main() {
	...
	logFile := getEnv("LOG_FILE", "./museums.log")
	...
	// Initialize museum service
	museumService := NewMuseumService(db)

	// Initialize handlers
	museumHandler := NewMuseumHandler(museumService)

	// Setup routes
	router := SetupRoutes(museumHandler)
