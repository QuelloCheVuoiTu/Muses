...
MONGO_COL = os.getenv('COLLECTION_NAME', 'node_info')
...
def setup_db_connection():
	...
	mongo = MongoClient(current_app)
	collection = mongo.db.get_collection(MONGO_COL)

def setup_db_connection_node()->Collection:
	mongo = MongoClient(f"mongodb://{MONGO_HOST}:{MONGO_PORT}/{MONGO_DB}")
	return mongo.db.get_collection("node")
...
def edit(id: str, data: dict) -> bool:
	...
	"""
	try:
		collection.update_one({"_id": ObjectId(id)}, {'$upsert': data})
	except Exception as e:
		logger.debug(e)
		return False
	return True
